...
export class SalesforceFormService implements ISalesforceFormService {
  ...
  private async getFromCacheOrFetch<T>(
    ...
    if (this.inFlightRequests.has(cacheKey)) {
      this.logger.log(`Using in-flight request for ${cacheKey}`);
    ...
    // Start fetching fresh data if:
    // 1. No cached data exists, or
    // 2. Cache is older than TTL/2 (stale-while-revalidate)
    const shouldFetch =
      !cachedData || (await this.isCacheStale(cacheKey, ttl / 2));
    ...
    if (shouldFetch) {
      ...
      if (cachedData) {
        this.logger.log(
          `Returning stale cache for ${cacheKey} while revalidating`,
        );
      ...
      // If no cached data, wait for the fetch
      return fetchPromise;
    }

    this.logger.log(`Retrieved from cache: ${cacheKey}`);
    return cachedData;
  }
  ...
  async getFormFields(
    ...
    const cacheKey = `form:${formId}:fields`;
